STM32 小狗复刻项目 🐶

复刻自Sngels_wyh大佬的开源项目，该项目详细记录了从淘宝购买“UP主推荐 STM32桌面宠物小狗 DIY 电子联系套件单片机开发板机器狗”后如何操作。
并上传了部分演示效果（是额外装了3D外壳后的，非常像一个正式的玩具机器狗！）

项目up主的视频教程 🎥
[新版教程链接🔗](https://www.bilibili.com/video/BV1XRhyzGE2E?spm_id_from=333.788.videopod.sections&vd_source=9799fa0a2eb3dd813b53aa4e7e030661)
该视频对应的合集包含了所有新旧版教程的讲解。

复刻顺序

1. 清点物料 🧾
   - 确认所有购买的物料是否完整。淘宝官方合作店铺购买后会有实体和电子两类资料，电子资料里有板子的原理图及各类视频资料指路。

2. 焊接过程 🔥
   - 注意事项：焊接时一定要避免虚焊或连锡，建议用尖头数显电烙铁，刀型电烙铁对新手焊接小元件时不太友好。
   - 焊接顺序：先焊贴片元件，再焊排针、排母等，不好焊接的话建议加助焊剂（松香）。
   - 注意LED正负极方向：请根据视频中指示焊接，随时测试功能是否正常。
   - 可以按部分焊接，焊好一部分通电测试下是否正常工作。

3. 烧录步骤 💻
   - 设备准备：确保电脑已安装好相应驱动。
   - 设备管理器：在设备管理器中找到对应的串口号，如 COM3。
   - 烧录过程：将程序下载到单片机里，注意插拔线。

4. 组装过程 🛠
   - 舵机安装：确保舵机的连接顺序正确，避免连接错误。
   - 测试运动：观察腿部运动情况，必要时调整连接线。

5. 调试与扩展 🖥
   - 动作提示：小狗进行前进动作时，注意幅度，避免晃动喇叭等元件，避免自动关机。
   - 可交互动作：小狗支持的交互动作有：
     - 前进 / 后退
     - 右转 / 左转
     - 叫 / 蹲下 / 趴下
     - 打招呼 / 伸懒腰 / 拉伸腿
     - 摇尾巴 / 摇摆
     - 向前跳 / 向后跳
     - 立正 / 睡觉


#声明 ⚠️
- 本项目仅供学习参考。

补充：
原淘宝商家发送的配套文件不做任何修改，可能存在以下问题：
1.摇尾巴可以通过再次下达“摇尾巴”命令后切换到别的指令，以此退出摇尾巴循环，但是摇摆不能通过同样方法退出
2.OLED屏需要保护好，如果小狗可以接受指令并做出动作，屏幕接线也正确，但是脸部屏幕不亮，需检查屏幕是否破损：即便是屏幕边角碎裂也可能影响屏幕是否可亮
3.烧录前一定完整观看配套视频，否则可能出现忘记“拔插线”、“选错端口”等问题
4.焊接充放电模块时一定要连接上PCB版和模块，不可以粘住就停焊
5.后续问题欢迎继续补充!
